"""Labeled variant of Figure 1: identical to the canonical semantic density
figure but with T* centroid labels overlaid on every topic.

Saves alongside the canonical version (does NOT overwrite it):
    figures/manuscript/figure_1_semantic_density_labeled.{png,pdf}
    jan_2026_manuscript/NMS2026/figures/figure_1_semantic_density_labeled.{png,pdf}
    jan_2026_manuscript/NatureCollection2026/figures/figure_1_semantic_density_labeled.{png,pdf}

Loads everything from the shared figure cache so PCA axes, point
positions, and topic assignments match every other semantic-map figure.
"""
...
def main() -> None:
    ...
    centroids = np.array([pub2d[labels_pub == k].mean(axis=0) for k in range(K)])

    print("Rendering Figure 1 (labeled variant)...")
    ...
    # T* centroid labels — small white-bg box for legibility on top of the
    # colored point cloud. Sit just above the centroid so the data point
    # remains visible.
    for k in range(K):
        cx, cy = centroids[k]
        ax.annotate(
            f"T{k}", xy=(cx, cy), xytext=(0, 6),
            textcoords="offset points",
            fontsize=8.5, fontweight="bold", color="#111111",
            ha="center", va="bottom", zorder=11,
            bbox=dict(
                boxstyle="round,pad=0.2",
                facecolor="white", edgecolor=TOPIC_COLORS[k],
    ...
    ax.set_title(
        "Semantic Density: Public Discourse vs Artist Concerns (labeled)",
        fontsize=12.5, fontweight="bold", pad=14,
    )
    ax.set_xlim(xr)
    ax.set_ylim(yr)
    plt.tight_layout()
    save_figure(fig, "figure_1_semantic_density_labeled")
    plt.close()
    print("Saved figure_1_semantic_density_labeled to all output dirs")
